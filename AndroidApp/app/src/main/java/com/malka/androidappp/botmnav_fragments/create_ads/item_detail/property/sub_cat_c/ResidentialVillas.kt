...
import kotlinx.android.synthetic.main.fragment_residential__villas.*
import kotlinx.android.synthetic.main.fragment_residential_holiday_house.*
import kotlinx.android.synthetic.main.fragment_residential_holiday_house.btnrholidayhouse


class ResidentialVillas : Fragment() {
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_residential__villas, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnrvillas.setOnClickListener() {
            val bedroom: String = bedrooms2.getText().toString()
            StaticClassAdCreate.bedrooms = bedroom

            val bathroom: String = bathrooms2.getText().toString()
            StaticClassAdCreate.bathrooms = bathroom

            val floorArea: String = floorAreaV2.getText().toString()
            StaticClassAdCreate.floorarea = floorArea

            val landArea: String = landArea2.getText().toString()
            StaticClassAdCreate.landarea = landArea

            //
            findNavController().navigate(R.id.residentialvillas_listingdetail)
        }
    }
}