...
class SelectedImagesAdapter(var data: List<ImageSelectModel>,var setOnSelectedMainImage:SetOnSelectedMainImage) :
    ...
    override fun onBindViewHolder(holder: SelectedImagesViewHolder, position: Int) {
        ...
        holder.viewBinding.isSelectCb.isChecked = data[position].is_main
        holder.viewBinding.isSelectCb.setOnCheckedChangeListener { buttonView, isChecked ->
            setOnSelectedMainImage.onSelectedMainImage(position,isChecked)
        }

    }

    interface SetOnSelectedMainImage{
        fun onSelectedMainImage(position: Int,isChecked:Boolean)
    }
}