...
import kotlinx.android.synthetic.main.fragment_residential_buildings.btnrbuildings
import kotlinx.android.synthetic.main.fragment_residential_holiday_house.*


class ResidentialHolidayHouse : Fragment() {
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_residential_holiday_house, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnrholidayhouse.setOnClickListener() {
            val bedroom: String = bedrooms8.getText().toString()
            StaticClassAdCreate.bedrooms = bedroom

            val bathroom: String = bathrooms8.getText().toString()
            StaticClassAdCreate.bathrooms = bathroom

            val floorArea: String = floorAreaV8.getText().toString()
            StaticClassAdCreate.floorarea = floorArea

            val landArea: String = landArea6.getText().toString()
            StaticClassAdCreate.landarea = landArea

            //

            findNavController().navigate(R.id.residentialholidayhouse_listingdetail)
        }
    }
}
