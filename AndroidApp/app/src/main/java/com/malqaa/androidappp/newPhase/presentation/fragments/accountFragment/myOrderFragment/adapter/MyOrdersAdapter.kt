...
class MyOrdersAdapter(
    ...
    override fun onBindViewHolder(holder: SoldOutOrdersViewHolder, position: Int) {
        ...
        holder.viewBinding.orderTimeTv.text = HelpFunctions.getViewFormatForDateTrackWithoutUTC(
            orderList[position].createdAt,
            "dd/MM/yyyy HH:mm:ss"
        );
        holder.viewBinding.shipmentsTv.text = orderList[position].providersCount.toString()
        holder.viewBinding.totalOrderTv.text =
            "${orderList[position].totalOrderAmountAfterDiscount} ${context.getString(R.string.rial)}"
